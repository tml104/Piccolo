...
void AnimationPose::blend(const AnimationPose& pose)
{
    for (int i = 0; i < m_bone_poses.size(); i++)
    {
        ...
        if (sum_weight != 0)
        {
            ...
            float w          = cur_weight / sum_weight;
            m_weight.m_blend_weight[i] = sum_weight;
            bone_trans_one.m_position = Vector3::lerp(bone_trans_two.m_position, bone_trans_one.m_position, w);
            bone_trans_one.m_scale    = Vector3::lerp(bone_trans_two.m_scale, bone_trans_one.m_scale, w);
            bone_trans_one.m_rotation =
                Quaternion::nLerp(w, bone_trans_two.m_rotation, bone_trans_one.m_rotation, true);
        }
    }
}
